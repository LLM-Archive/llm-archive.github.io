...
file needs Croissant's fileSet, not fileObject, kept out for now rather than guessed) for tools
that read the MLCommons Croissant format. Every field's Croissant dataType comes from the same
MEASUREMENT_FIELDS/OUTCOME_FIELDS types this module already uses (pct/unit_interval -> sc:Float,
int/count -> sc:Integer, bool -> sc:Boolean, everything else -- including every JSON-cell field --
-> sc:Text) -- one small, fixed mapping, not a per-field decision. Doesn't read experiments/ at
all: unlike the CSVs, this file only describes their shape, so it never changes unless the schema
itself does.

Deliberately not doing yet: coverage.csv (no coverage_gap-producing code exists), the open-lane
fileSet in croissant.json, the HTML pages, the public/private split itself. Each is a separate
piece.
"""
...
# module must not duplicate it. Still not doing: coverage.csv (blocked -- see the module
# docstring above), the open-lane croissant fileSet, or the actual publish/deploy step.
...
def load_active_commercial_model(path: Path) -> dict | None:
    """Hand-parses subject_models.yaml's commercial series for a generation with status
    "active" -- same "extract only the one shape this needs" approach as
    core/schedule/schedule.py's cadence.yaml reader, so this module doesn't need a YAML dependency
    for one field. Returns None if the file is missing or no commercial generation is active yet
    (true today -- see the file's own comment: model_id stays null until the first real run)."""
    if not path.exists():
        return None
    commercial_block = _indented_block(path.read_text(encoding="utf-8").splitlines(), "commercial:")
    if not commercial_block:
        return None
...
def build_site_data(experiments_dir: Path, protocols_dir: Path, subject_models_path: Path) -> dict:
...
def render_file_rows(data_dir: Path, out_dir: Path, open_lane_years: list[str]) -> str:
    ...
    rows.append(
        '<tr><td><code>coverage.csv</code></td><td>What was measured, what\'s missing, and why</td>'
        '<td class="num">not yet available</td><td class="hash">—</td></tr>'
    )
    return "\n".join(rows)
...
def build_site_html(
    ...
    out_dir: Path | None = None, version_path: Path = DEFAULT_VERSION_FILE,
) -> str:
    data = build_site_data(experiments_dir, protocols_dir, subject_models_path)
...
def write_site_html(
    ...
    html = build_site_html(experiments_dir, protocols_dir, subject_models_path, data_dir, template_path, out_path.parent, version_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def _verify() -> list[str]:
    ...
    if file_ids != {"stability.csv", "outcomes.csv"}:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "build-site":
        write_site_html(args.experiments_dir, args.protocols_dir, args.subject_models, args.data_dir, args.out, args.template, args.version_file)
        print(f"wrote {args.out}")
        return 0
