...
**Current status: one real measurement exists, nothing is public yet.** The pipeline has run
...
run against it yet. There is no public repository or website yet — the code lives in a private
repo, and the public site repo hasn't been created. Everything below describes how the finished
project is designed to work. Where something described here isn't built yet, it's marked as such
— this project's own principle (`docs/spec.md` §9, §10) is to disclose gaps rather than imply a
feature exists before it does, and this guide follows the same rule about itself.
